...
- **Tailnet HTTPS exposure:** serving the UI over the tailnet (serve → loopback) means the serve rule is config **separate from the systemd unit** — it can be repointed at a throwaway file server, making every session deep-link open a directory listing while the UI itself runs fine. When remote links misbehave, check the serve rule before the server. Verify with GET, not HEAD (some proxies handle them differently).
