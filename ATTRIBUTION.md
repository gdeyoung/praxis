...
| Dangerous-command regex denylist (adapted for Linux) | **David Ondrej** | [github.com/davidondrej/skills](https://github.com/davidondrej/skills) | Starting point for our global command-guard pattern list ([plugin-seam.md](hermes/plugin-seam.md), Plugin 3). We ported patterns, added fleet-specific entries, and changed the integration to a pre-execution hook. MIT. |
| sched_ext + ananicy-cpp desktop latency tier (scx_lavd) | **neur0map / ryoku-dev** | [ryoku.dev](https://ryoku.dev/) · [github.com/Ryoku-dev/ryoku-arch](https://github.com/Ryoku-dev/ryoku-arch) | The perf tier in our Omarchy recipe ([recipe-doctor.md](hermes/recipe-doctor.md) host context) was lifted directly from Ryoku Linux's approach: sched_ext scheduler plus per-process nice-level management on Arch. GPL-3. |
